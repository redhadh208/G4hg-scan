...
class OBDService {
  ...
  async connect(address, options = {}) {
    return await BluetoothService.connect(address, options);
  }
  ...
  startReading(callback, rate = 1500) {
    ...
    const run = async () => {
      ...
      try {
        const data = await BluetoothService.readAllPIDs();
  ...
  async generateHealthReport(liveData) {
    ...
    const check = (key, value, label, min, max, unit) => {
      if (value === 0 || value === null) {
        warnings.push({ sensor: label, status:'NO_DATA', msg:'Aucune donnée', value:'N/A', unit });
