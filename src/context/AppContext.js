...
function reducer(state, action) {
  switch (action.type) {
    case 'SET_CONNECTION':
      return { ...state, isConnected: action.payload, connectedDevice: action.device, connectionMode: action.mode || state.connectionMode };
    case 'UPDATE_LIVE':
...
export function AppProvider({ children }) {
  ...
  const actions = {
    setConnection: (ok, device = null, mode = 'bluetooth') => dispatch({ type: 'SET_CONNECTION', payload: ok, device, mode }),
