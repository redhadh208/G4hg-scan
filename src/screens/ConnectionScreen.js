...
import { INJECTOR_SPECS, FUEL_REGULATOR } from '../utils/sensorReference';
...
export default function CorrectionScreen() {
  ...
  const applyFuelPressure = async () => {
    ...
    Alert.alert(`⚠️ Pression cible ${val} kPa`,`Constructeur: 350 kPa\nNouvelle cible: ${val} kPa\n\nContinuer ?`,
      [{text:'Annuler',style:'cancel'},{text:'Appliquer',style:'destructive',onPress:async()=>{
        setLoading(true);
  ...
  return (
    <ScrollView style={st.container}>
          ...
          return (
            <View key={inj.id} style={st.injRow}>
              <View style={{flex:1}}>
                ...
                <Text style={[st.injLive,{color:liveCol}]}>Actuel: {live?live.toFixed(2)+'ms':'N/A'} {live&&(live>=2&&live<=3.5)?'✅':'⚠️'}</Text>
                <Text style={st.injOffset}>Offset: {offset>0?'+':''}{offset} mg/coup</Text>
              </View>
              <View style={st.injBtns}>
                <Btn label="-1" color={C.red} small onPress={()=>{const nv = Math.max(inj.minOffset, offset-1); setInjOffsets(p=>({...p,[inj.id]:nv}));}}/>
                <Text style={st.injOffsetBig}>{offset>0?'+':''}{offset}</Text>
                <Btn label="+1" color={C.green} small onPress={()=>{const nv = Math.min(inj.maxOffset, offset+1); setInjOffsets(p=>({...p,[inj.id]:nv}));}}/>
      ...
      <View style={st.card}>
        <Text style={st.cardInfo}>Type: Return-less · Cible constructeur: 350 kPa (3.5 bar){'\n'}Ralenti: 330-370 kPa</Text>
        <View style={st.liveRow}><Text style={st.liveLabel}>Pression actuelle:</Text><Text style={[st.liveVal,{color:d.fuelPressure>=330&&d.fuelPressure<=370?C.green:C.red}]}>{d.fuelPressure?d.fuelPressure.toFixed(0)+'kPa':'N/A'}</Text></View>
        <View style={st.inputRow}><Text style={st.inputLabel}>Pression cible (kPa):</Text><TextInput style={st.input} value={fuelTarget} onChangeText={setFuelTarget} keyboardType="numeric" placeholderTextColor={C.muted}/></View>
      ...
      <View style={st.card}>
        ...
        <View style={st.liveRow}><Text style={st.liveLabel}>RPM actuel:</Text><Text style={[st.liveVal,{color:d.rpm>=700&&d.rpm<=850?C.green:C.red}]}>{d.rpm?d.rpm.toFixed(0)+' rpm':'N/A'}</Text></View>
        <View style={st.inputRow}><Text style={st.inputLabel}>RPM cible:</Text><TextInput style={st.input} value={rpmTarget} onChangeText={setRpmTarget} keyboardType="numeric" placeholderTextColor={C.muted}/></View>
        <View style={{flexDirection:'row',gap:8,marginTop:8}}>
      ...
      <View style={[st.card,{borderColor:C.red+'40'}]}>
        <Text style={{color:C.red,fontSize:13,marginBottom:12,lineHeight:20}}>⚠️ Remet TOUT aux valeurs d'origine constructeur:{'\n'}• Injecteurs: offsets IQA = 0{'\n'}• Pression carburant: 350 kPa{'\n'}• RPM ralenti: 750 rpm{'\n'}• Adaptations LTFT/STFT effacées</Text>
        <Btn label="🏭 TOUT REMETTRE AUX VALEURS CONSTRUCTEUR" color={C.red} onPress={fullFactoryReset} disabled={loading}/>
      </View>

      {loading && (<View style={st.overlay}><ActivityIndicator size="large" color={C.accent}/><Text style={{color:C.accent,marginTop:12,fontSize:14}}>Communication ECM...</Text><Text style={{color:C.muted,fontSize:11,marginTop:4}}>Ne pas couper le contact</Text></View>)}
